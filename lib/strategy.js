...
function WechatStrategy(options, verify) {
    ...
    if (!options.appid || !options.clientID) {
        throw new TypeError('WechatStrategy requires a appid/clientID option');
    }

    if (!options.appsecret || !options.clientSecret) {
        throw new TypeError('WechatStrategy requires a appsecret/clientSecret option');
    }
    ...
    this.name = 'wechat';
    this._verify = verify;
    this._oauth = new OAuth(options.appid || options.clientID, options.appsecret || options.clientSecret);
